...
class CalculatorTest {
    ...
    public void maxOfThreeNumbersFirst() {
        ...
        Calculator calculator = new Calculator();
        int result = calculator.maxOfThreeNumbersFirst(firstNumber, secondNumber, thirdNumber);

        check(firstNumber, result, "maxOfThreeNumbersFirst");
    }

    private void check(int expectedResult, int actualResult, String testName) {
        if (expectedResult == actualResult) {
            System.out.println("Dev test = OK. Actual result is 2 and expected result is " + testName + ".");
        } else {
            System.out.println("Dev test = FAIL. Actual result is 2, but expected result is " + testName + ".");
        }
    }
